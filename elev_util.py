...
if 1:
    ...
    # else:
    #     from ..oee.utils.elev_est_api import elev_est_api
    from oee.utils.elev_est_api import elev_est_api,ElevEstHelper,elev_est_api_bsz
import  numpy  as np
...
def eleRadian_2_baseXyz_lXyz(eleRadian:float):#xyz is in degree!
    ...
    l_ele_azimuth_inRadian=_get_l_ele_azimuth_inRadian(
        # num_samples=128
        num_samples=confs.NUM_REF
    )
    if confs.ELEV_RANGE:#only keep elev in ELEV_RANGE
        assert len(confs.ELEV_RANGE)==2
        # l_ele_azimuth_inRadian=np.array(l_ele_azimuth_inRadian)
        # to degree
        l_ele_azimuth_inDeg=np.rad2deg(l_ele_azimuth_inRadian)
        l_ele_azimuth_inDeg=l_ele_azimuth_inDeg[l_ele_azimuth_inDeg[:,0]>=confs.ELEV_RANGE[0]]
        l_ele_azimuth_inDeg=l_ele_azimuth_inDeg[l_ele_azimuth_inDeg[:,0]<=confs.ELEV_RANGE[1]]
        # to radian
...
def imgPath2elevRadian(K,input_image_path,run4gen6d_main,id_):
    ...
    output_dir = os.path.join(confs.dataPath_rfdb, f'{id2}/ref')
    def getFourNearImagePaths( _id=id2):
        # delta_x_2 = [-10, 10, 0, 0]
        # delta_y_2 = [0, 0, -10, 10]
        DELTA=10
    ...
    if confs.force_elev:
        print(f"[warning] You enable {confs.force_elev=}")
        assert confs.DATASET=='gso','暂时应该只有gso可以获得GT elev吧'
        fourNearImagePaths='(force_elev)'
        elev=confs.force_elev
    else:
        if 1: # confs.elev_bsz.enable branch
            ib_2_fourNearImagePaths=[]
            for ib in range(confs.elev_bsz.BSZ):
                _id=f"{id2}--{ib}"
                fourNearImagePaths=getFourNearImagePaths(_id)
                ib_2_fourNearImagePaths.append(fourNearImagePaths)
            elev=elev_est_api_bsz( ib_2_fourNearImagePaths,
                                # min_elev=30, max_elev=150,
                                # min_elev=20, max_elev=160,
                                min_elev=90-79, max_elev=90-0, #NOTE that 'elev' here is angle to z-axis; 注意root_config.Q0_MIN_ELEV
