...
import  torch
class Estimator4Co3dEval(): # toname
    """
    ...
    def __init__(
        ...
        id_ =refIdWhenNormal
        s.id_ = id_
        s._ref_database_folder=os.path.join(confs.dataPath_gen6d, f'{id_}/ref')
    ...
    def get__zero123Input_info(self,K,image0_path):
        ...
        if confs.one_SEQ_mul_Q0__one_Q0_mul_Q1:
            ...
            tmp1=os.path.relpath(info["img_path__origin___beforeIPR"],confs.path_root)
            tmp2=os.path.relpath(image0_path,confs.path_root)
            if confs.CONF_one_SEQ_mul_Q0__one_Q0_mul_Q1.ONLY_CHECK_BASENAME:
    ...
    def __save_zero123Input_img_and_info(
        ...
        '''
        #relToRoot
        img_path_relToRoot__origin=os.path.relpath(confs.path_root,img_path__origin)
        img_path_relToRoot__in_ref=os.path.relpath(confs.path_root,img_path__in_ref)
        info={ 
    ...
    def tmp_gen_zero123Input_info_for_old_ref_database__save_zero123Input_img_and_info(self,img_path__origin,elevRadian):
        return self.__save_zero123Input_img_and_info(img_path__origin.replace("/root/autodl-tmp/cv",confs.path_root+"/"),elevRadian)
    def __run_zero123(s,K, input_image_path, input_image_eleRadian: float, id_):
        ...
        def save__ref_relaToBase_info(base_xyz, l_xyz ):
            ...
            for x_relToBase,y_relToBase,_ in l_xyz:
                ...
                assert abs(delta_elev)<(90-confs.Q0_MIN_ELEV)+0.01,f"delta_elev={delta_elev}"
    ...
    def __build_ref_and_init_sparseViewPoseEstimator(s,K,q_img_path0,q_img_eleRadian=None, ):
        ...
        if confs.SKIP_GEN_REF_IF_REF_FOLDER_EXIST and ref_folder_build_OVER.exists():
            ...
            if confs.CONSIDER_IPR:
                ...
                # img_path__origin___beforeIPR="!confs.CONSIDER_IPR==False"
                img_path__origin___beforeIPR=zero123_input_img_path
