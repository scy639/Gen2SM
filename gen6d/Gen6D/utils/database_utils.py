...
# if(os.path.abspath(os.path.join(os.path.dirname(__file__),os.path.pardir))   in sys.path):
from imports import *
...
def look_at_crop(img, K, pose, position, angle, scale, h, w):
    ...
    pose_new = pose_compose(pose, pose_rect)# world->camera-img_new = camera-img->camera-img_new @ world->camera-img
...
def compute_normalized_view_correlation(que_poses, ref_poses, center, th=True):
    ...
    if th:
        ...
        que_cams = (que_poses[:, :, :3].transpose([0, 2, 1]) @ -que_poses[:, :, 3:])[..., 0]  
...
def normalize_reference_views(database, ref_ids, size, margin,
                              ...
                              save_normalized_ref_imgs=False,
                              ):
    object_center = get_object_center(database)
    object_diameter = get_diameter(database)  # ==2
    ...
    ref_cens = np.asarray(
        [project_points(object_center[None], pose, K)[0][0] for pose, K in zip(ref_poses, ref_Ks)])  # rfn,2
    ...
    ref_focal_look = np.asarray(
        [let_me_look_at(pose, K, object_center)[1] for pose, K in zip(ref_poses, ref_Ks)])  # rfn
    ...
    if rectify_rot:
        if input_K is not None and input_pose is not None:
            ...
            _, ref_vert_angle = scale_rotation_difference_from_cameras(ref_poses, input_pose, ref_Ks, input_K,
                                                                       object_center)  # rfn
        else:
    ...
    for k in range(len(ref_ids)):
        ...
        if add_rots:
            ref_img_rot = np.stack([look_at_crop(ref_img, ref_Ks[k], ref_poses[k], ref_cens[k], ref_vert_angle[k] + rot,
        ...
        if (save_normalized_ref_imgs):
            output_dir = gen6dGlobal.output_dir
            tmp_dir=f'{str(output_dir)}/normalized_ref_imgs'
            if not os.path.exists(tmp_dir):
                os.makedirs(tmp_dir)
            imsave(f'{tmp_dir}/ref_ids[k={k}]={ref_ids[k]}.jpg', ref_img_new)
    ...
    ref_imgs_new, ref_Ks_new, ref_poses_new, ref_Hs, ref_masks_new = \
        np.stack(ref_imgs_new, 0), np.stack(ref_Ks_new, 0), np.stack(ref_poses_new, 0), np.stack(ref_Hs, 0), np.stack(
            ref_masks_new, 0)
...
def select_reference_img_ids_fps(database, ref_ids_all, ref_num, random_fps=False):
    ...
    assert ref_num==len(ref_ids_all),f"ref_num, ref_ids_all ={ref_num},{ ref_ids_all}"
    idxs=np.array(list(range(ref_num)))

    ref_ids = np.asarray(ref_ids_all)[idxs]  # rfn
    # 
    
    # assert Global.ref_idxs==None
    # Global.ref_idxs=idxs
    return ref_ids


def select_reference_img_ids_refinement(ref_database, object_center, ref_ids, sel_pose, refine_ref_num=6,
                                        refine_even_ref_views=False, refine_even_num=128):
    ...
    if(confs.USE_ALL_SAMPLE):
        N=len(ref_database.img_ids)//confs.SAMPLE_NUM
        ref_idxs=[idx for idx in ref_idxs if idx<N]
    else:
