...
class Gen6DEstimator:
    ...
    def _load_module(cfg):
        ...
        if refiner_cfg['network']=='selector':
            pth_path=confs.weightPath_selector
            del refiner_cfg["name"]
        elif refiner_cfg['network']=='refiner':
            pth_path=confs.weightPath_refiner
            del refiner_cfg["name"]
        else:
        ...
        state_dict = torch.load(pth_path,map_location=torch.device(confs.DEVICE))
    ...
    def predict(self, que_img, que_K, pose_init=None,
        ...
        if self.refiner is not None:
            ...
            for k in range(  confs.REFINE_ITER  ):
