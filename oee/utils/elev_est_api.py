...
class ElevEstHelper:
    ...
    def get_feature_matcher(cls):
        if cls._feature_matcher is None:
            ...
            ckpt_path=confs.weightPath_loftr
...
def find_optim_elev(elevs, nimgs, matches, K, dbg=False,for_IPR=False):
    ...
    return optim_elev,min_err,errs
...
def get_elev_est(feature_matching, min_elev=30, max_elev=150, K=None, for_IPR=False):
    ...
    optim_elev1 ,_,_= find_optim_elev(elevs, N, matches, K,for_IPR=for_IPR)

    elevs = np.arange(optim_elev1 - 10, optim_elev1 + 10, 1)
    optim_elev2,min_err,_ = find_optim_elev(elevs, N, matches, K,for_IPR=for_IPR)
...
def elev_est_api(img_paths, 
    ...
    elev=90-elev
    return elev
# def elev_est_api_bsz(ib_2_feature_matching, min_elev=30, max_elev=150, K=None, for_IPR=False):
def elev_est_api_bsz(ib_2_img_paths,
                #  min_elev=30, max_elev=150,
                 min_elev, max_elev,
                 K=None, dbg=False):
    ib_2_feature_matching=[]
    for ib,img_paths in enumerate(ib_2_img_paths):
        feature_matching = get_feature_matching(img_paths, dbg=dbg)
        ib_2_feature_matching.append(feature_matching)
    if K is None:
        loguru.logger.warning("K is not provided, using default K")
        K = np.array([[280.0, 0, 128.0],
                      [0, 280.0, 128.0],
    ...
    #---------------------------------------structure same as get_elev_est
    for_IPR=False
    ib_2_matches=[]
    for ib,feature_matching in enumerate(ib_2_feature_matching):
        flag = True
        matches = {}
        if for_IPR:
            N=len(IPR_Delta_Elev_Azim.l_elev_azim)
        ...
        if not flag:
            ...
            if for_IPR:
                ...
                continue
        ib_2_matches.append(matches)
    if len(ib_2_matches)==0:
        print('warning:: len(ib_2_matches)==0')
        return 45
    def _f(elevs_):
        ib_2_errs = []  # errs is a tensor
        for ib, matches_ in enumerate(ib_2_matches):
            optim_elev, _, errs = find_optim_elev(elevs_, N, matches_, K, for_IPR=for_IPR)
            ib_2_errs.append(errs)
        sum_errs=torch.stack(ib_2_errs).sum(0)
        print(f"{ib_2_errs=}")
        print(f"{sum_errs=}")
        assert sum_errs.shape==ib_2_errs[0].shape
        min_err = min(errs).item()
        # print(f"[find_optim_elev]min_err={min_err}")
        optim_elev = elevs_[torch.argmin(sum_errs)].item()
        return optim_elev,min_err
    interval = 10
    elevs = np.arange(min_elev, max_elev, interval)
    optim_elev1,_=_f(elevs)
    elevs = np.arange(optim_elev1 - 10, optim_elev1 + 10, 1)
    if 10 in elevs:#if 10 in elevs, remove 10
        print('if 10 in elevs, remove 10')
        elevs = np.delete(elevs, np.where(elevs == 10))
    optim_elev2,min_err = _f(elevs, )
    print(f"optim_elev2,min_err= {optim_elev2,min_err}")
    if for_IPR:
        ret= min_err
    else:
...
def imgPath2IPRbyOEE(
    ...
    def get_min_err(path_rot,batchB=False):
        ...
        def getNearImagePaths(path_rot:str):
            ...
            in zero123: path_output_ims = os.path.join(confs.dataPath_zero123, parentFolderName_output_ims,id)
            """
    ...
    def _f3(l_angle_in_degree):
        ...
        for rot_angle_in_degree in l_angle_in_degree:
            ...
            if confs.ForDebug.forceIPRtoBe.enable is True:
                min_err = -639
            else:
    ...
    def normalize_deg(x):
        ...
        return x%360
    if confs.ForDebug.forceIPRtoBe.enable is True:
        l_angle_in_degree =[confs.ForDebug.forceIPRtoBe.IPR]
    else:
        if 1:
            _f3_=_f3_batchB
        else:
        ...
        l_angle_in_degree=[normalize_deg(x) for x in l_angle_in_degree]
        coarse_deg,_=_f3_(l_angle_in_degree)
        l_angle_in_degree = (-6,-4,-2,0,2,4,6,)#len=7-1
