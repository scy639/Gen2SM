...
class NaviDatabase(BaseDatabase) :
    ...
    def __init__(self, obj_with_scene :str,):
        ...
        scene = scene.replace(')','')
        #
        DATASET_ROOT =  os.path.abspath  ( confs.dataPath_navi)
        ...
        _tmp1=folder__images_after_exif_transpose
        ...
        annotations, _, image_names = load_scene_data(
            ...
            folder__images_after_exif_transpose=_tmp1,
        )
        del _tmp1
    ...
    # def get_elev_azim_inDeg(self,img_id): # GT elev,azim. just for debug
    #     pose34=self.get_pose(img_id)
    #     camera_z=pose34[2][:3]#camera z's coord in the refdb world coord system
    #     # print(f"{camera_z=}")
    #     if 1:
    #         elev_degree=math.degrees(math.asin(-camera_z[1]))
    #         azim_degree=math.degrees(math.atan2(camera_z[0],camera_z[2]))
    #     azim_degree%=360
    #     #
    #     round_arg=3
...
class NaviDataset(BaseDatasetWrapper):
