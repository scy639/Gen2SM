...
class BaseDatasetWrapper(BaseDataset):
    def __init__(
            self,
            category:str ,
    ):
    ...
    def _get_from_database(
        self, index0, index1,
        q0ipr,q1ipr, # img0,1 's in-plane rotation angle
        max_size=None, mask_out=True,
        ...
        for j,index in enumerate(_index_pair):
            ...
            if 1:
                # elev_deg,az_deg=database.get_elev_azim_inDeg(index)
            ...
            if 1:
                ...
                if confs.MASK_ABLATION=='EROSION':
                    ...
                    assert confs.MASK_ABLATION is None
            ...
            # if j==0:
            if 1:
                if confs.MASK_ABLATION is not None:
                    ttt426=image_full_path
                    image_full_path=f"{image_full_path}_{confs.MASK_ABLATION[:3]}.png"
            ...
            l_bbox.append(bbox)
            Ks.append(K)
        return images_not_transformed,image_full_paths,rotations,translation31s,pose44s,l_bbox,Ks,l_elev_deg,l_az_deg
    def get_data_(self, sequence_name, ids  ,q0ipr=0,q1ipr=0,):
        ...
        only need these field in batch:
            ...
            4. K
        """
        images_not_transformed, image_full_paths, rotations, translation31s, pose44s,l_bbox ,Ks,l_elev_deg,l_az_deg= self._get_from_database(ids[0],ids[1],q0ipr,q1ipr,)
        detection_outputs=[]
        ...
        batch={}
        if(not confs.LOOK_AT_CROP_OUTSIDE_GEN6D):
        ...
        batch["elev_deg"] = l_elev_deg
        batch["azim_deg"] = l_az_deg
        def vis_4debug():
            ...
            tmp_img = cv2_util.concat_images_list(
                tmp_poseVisualizer.get_img__mulView_A(
                    ...
                    y_is_vertical=1,
                    title=f"ids,image_full_paths={ids},\n{[os.path.relpath(_,confs.path_root) for _ in image_full_paths]}"
                ),
            ...
            debug_imsave(f'omni-get_data_/ids={ids}.jpg',tmp_img,)
        # vis_4debug()
        ...
        def batch_img_2_batch_img_full_path(batch_img):
            ...
            for i in range(2):
                ...
                tmp_cate_or_obj=self.database.cate if 0 else self.database.obj
                if self.__class__.__name__=='NaviDataset':
                    tmp_cate_or_obj=self.database._obj_with_scene
                """
                
                """
                if self.__class__.__name__=='CustomDataset':
                    pass
                elif self.__class__.__name__=='NaviDataset':
                    pass
                else:
                    assert img_name_without_dir.endswith('.png'),'database出来的图就是jpg,去看看是不是有损，影响zero123'
                if confs.SHARE_tmp_batch_images :
                    assert isinstance(confs.SHARE_tmp_batch_images,str) 
                    tmp_images_subfolderName=confs.SHARE_tmp_batch_images
                else:
                    assert confs.SHARE_tmp_batch_images is False
                    tmp_images_subfolderName=f'{os.getpid()}-{threading.currentThread().getName()}'
                full_path = os.path.join(  confs.dataPath_tmp_batch_images  , f'{tmp_images_subfolderName}/[{confs.DATASET}][{tmp_cate_or_obj}][{sequence_name}]{img_name_without_suffix}{confs.tmp_image__SUFFIX}')
                os.makedirs(os.path.dirname(full_path),exist_ok=True)
                pDEBUG("get_data path:", full_path)
                if confs.SHARE_tmp_batch_images and os.path.exists(full_path):
