...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.logvar=self.logvar.cuda()#torch 1.12->1.13   https://github.com/pytorch/pytorch/releases/tag/v1.13.0
...
class LatentDiffusion(DDPM):
    ...
    def instantiate_first_stage(self, config):
        ...
        self.first_stage_model = model.eval()#scy: ldm.models.autoencoder.AutoencoderKL
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        CACHE_ENABLE=root_config.cache_4_ldm_get_input.enable
        if CACHE_ENABLE:
            cache_4_ldm_get_input=root_config.cache_4_ldm_get_input
            cur_cond_id=cache_4_ldm_get_input.cur_cond_id
            cur_target_id=cache_4_ldm_get_input.cur_target_id
        #--------1. z---------
        if CACHE_ENABLE:
            z = cache_4_ldm_get_input.target_2_z.get(cur_target_id,None)
        else:
            z=None
        if z is None:
            if kwargs.get ('scy_target_is_z_insteadof_rgb',False):
                z = batch['z_target']
            else:
                ...
                z = self.get_first_stage_encoding(encoder_posterior).detach()
            if CACHE_ENABLE:
                cache_4_ldm_get_input.target_2_z[cur_target_id] = z
        #---------2. xc------------
        if CACHE_ENABLE:
            xc = cache_4_ldm_get_input.cond_2_xc.get(cur_cond_id,None)
        else:
            xc=None
        if xc is None:
            ...
            if bs is not None:
                xc = xc[:bs]
            if CACHE_ENABLE:
                cache_4_ldm_get_input.cond_2_xc[cur_cond_id]=xc
        cond = {}
        ...
        with torch.enable_grad():
            # ---------3. clip_emb ------------
            if CACHE_ENABLE:
                clip_emb = cache_4_ldm_get_input.cond_2_clip_emb.get(cur_cond_id, None)
            else:
                clip_emb = None
            if clip_emb is None:
                clip_emb = self.get_learned_conditioning(xc).detach()
                if CACHE_ENABLE:
                    cache_4_ldm_get_input.cond_2_clip_emb[cur_cond_id] = clip_emb
            # null_prompt = self.get_learned_conditioning([""]).detach()
            cond["c_crossattn"] = [self.cc_projection(torch.cat([clip_emb, T[:, None, :]], dim=-1))]
        # ---------3. c_concat ------------
        if CACHE_ENABLE:
            c_concat = cache_4_ldm_get_input.cond_2_c_concat.get(cur_cond_id,None)
        else:
            c_concat=None
        if c_concat is None:
            c_concat= self.encode_first_stage((xc.to(self.device))).mode().detach()
            if CACHE_ENABLE:
                cache_4_ldm_get_input.cond_2_c_concat[cur_cond_id]=c_concat
    ...
    def encode_first_stage(self, x):
        if hasattr(self, "split_input_params"):#scy:0
            if self.split_input_params["patch_distributed_vq"]:
                ks = self.split_input_params["ks"]  # eg. (128, 128)
                stride = self.split_input_params["stride"]  # eg. (64, 64)
    ...
    def p_losses(self, x_start, cond, t, noise=None,q0_noisePr=None,scy_only_ret_PR=False,**kw):#scy comment all loss_dict related
        if root_config.lBB.enable:
            assert q0_noisePr is not None
            assert self.parameterization == "eps"
            x_noisy=x_start
            assert noise is '639'
            noise=q0_noisePr
        else:
            if kw.get ('scy_dont_add_noise',False):
                x_noisy=x_start
            else:
                noise = default(noise, lambda: torch.randn_like(x_start))
                x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        ...
        if scy_only_ret_PR:
            return model_output #在q1 ddpm时又会作为q0_noisePr传进来.

        print_tensor_info(model_output,  ' ',0.002)
        loss_dict = {}
        ...
        loss_simple = self.get_loss(model_output, target, mean=False).mean([1, 2, 3]) # b,
        ...
        if 1:
            BATCH_multiple_iv = root_config.BATCH_multiple_iv
            bIV = root_config.bIV
        if not BATCH_multiple_iv:
            loss = self.l_simple_weight * loss.mean()
        else:
            loss = self.l_simple_weight * loss # bIV*bsz,
        ...
        if BATCH_multiple_iv:
            bsz = len(loss) // bIV
            loss = loss.view(bIV,bsz) # bIV*bsz,->
            loss = loss.mean(dim=1) # -> bIV,
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        super().__init__()
        self.diffusion_model = instantiate_from_config(diff_model_config)# scy: ldm.modules.diffusionmodules.openaimodel.UNetModel
