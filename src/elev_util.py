...
def imgPath2elevRadian(K,input_image_path,run4gen6d_main,id_):
    ...
    output_dir = os.path.join(root_config.dataPath_rfdb, f'{id2}/ref')
    ...
    if root_config.Cheat.force_elev:
        ...
        assert root_config.DATASET=='gso','暂时应该只有gso可以获得GT elev吧'
        fourNearImagePaths='(Cheat.force_elev)'
        elev=root_config.Cheat.force_elev
    else:
        if root_config.elev_bsz.enable:
            ib_2_fourNearImagePaths=[]
            for ib in range(root_config.elev_bsz.BSZ):
                _id=f"{id2}--{ib}"
