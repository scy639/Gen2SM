        ...
        for i_pair, (i, j) in enumerate(pairs):
            def eval_pair():
                ...
                if not root_config.LOOK_AT_CROP_OUTSIDE_GEN6D:
                    ...
                    refId1 = root_config.RefIdWhenNormal.get_id(
                        category, sequence_name,
                        root_config.refIdSuffix,
                        j
                    )
                    ldbId0 = root_config.RefIdWhenNormal.get_id(
                        category, sequence_name,
                        root_config.ldbIdSuffix,
                        i
                    )
                    ldbId1 = root_config.RefIdWhenNormal.get_id(
                        category, sequence_name,
                        root_config.ldbIdSuffix,
                        j
                    )
                    ...
                    if 0: # E2VG
                        if 'GT_elev0' in Global.anything:
                            root_config.Cheat.force_elev = batch['elev_deg'][0]
                        ...
                        R_pred_rel, T31_pred_rel, relative_pose, inter = infer_pair_latent.infer_in_latent_space_B(
                ...
                def _vis():
                    def vis_pose():
                        ...
                        l_color.append("grey")
                        title += f".elev={tmp_eleRadian * 180 / math.pi:.2f}°{q0ipr=}{q1ipr=}\nzero123 base img={Path(zero123_input_img_path).relative_to(root_config.dataPath_rfdb)}"  # azim=-60, elev=30 by default
