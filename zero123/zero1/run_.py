...
_GPU_INDEX = confs.GPU_INDEX

CONFIG='configs/sd-objaverse-finetune-c_concat-256.yaml'
device = f'cuda:{confs.GPU_INDEX}'
...
def sample_model(input_im, 
    ...
    with precision_scope(confs.DEVICE):
...
def sample_model_batchB_wrapper(
    ...
    for zero123_BatchB_Input in l__zero123_BatchB_Input:
        ...
        if 1:
            ...
            path_output_ims = os.path.join(confs.dataPath_zero123, folder_outputIms,id_)
            del folder_outputIms
        else:
            path_output_ims = os.path.join(confs.dataPath_zero123, id_)
        ...
        for i, (x, y, z) in enumerate(l_xyz):
            ...
            if not os.path.exists(img_save_path) or confs.FORCE_zero123_render_even_img_exist:
    ...
    #-----------------------
    ModelsGetter.set_param(f'cuda:{confs.GPU_INDEX}', confs.weightPath_zero123, CONFIG)
    torch.cuda.empty_cache()  # 
    models = ModelsGetter.get_B()
    sampler = DDIMSampler(models['turncam'])
    sample_batch_B_size = confs.SAMPLE_BATCH_B_SIZE
...
NO_CARVEKIT=confs.NO_CARVEKIT    #     zero123 no remove bg again (no_carvekit)
...
def main_run(models, device, cam_vis, return_what,
    ...
    # debug_imsave(f'e2vg_run1-preprocess_image/{confs.refIdSuffix}--{confs.idSuffix}/{NO_CARVEKIT}-before--{get_datetime_str()}.jpg',raw_im)
    ...
    if 'vis' in return_what:
        ...
        if (batch_sample):
            ...
            sample_batch_size=confs.SAMPLE_BATCH_SIZE
            ...
            #     print('confs.LOG_WHEN_SAMPLING==false...')
            ...
            # if(not confs.LOG_WHEN_SAMPLING):
...
def sample_model_(
    ...
    if 'parentFolderName_output_ims' in kw:
        ...
        path_output_ims = os.path.join(confs.dataPath_zero123, parentFolderName_output_ims,id)
    else:
        path_output_ims = os.path.join(confs.dataPath_zero123, id)
    if not os.path.exists(path_output_ims):
        os.makedirs(path_output_ims)
