...
def eval_on_an_obj(
    ...
    evalResult = EvalResult(model_name=model_name, idSuffix=confs.idSuffix, category=category)

    device = confs.DEVICE  
    if confs.DATASET == "gso":
        dataset = GsoDataset(category=category)
    elif confs.DATASET == "navi":
    ...
    for sequence_name in dataset.sequence_list:
        if confs.Q0Sipr:
            ...
            q0ipr=map_string_to_int(category+sequence_name+str(confs.Q0INDEX),
                                    # -45,45)
                                    -confs.Q0Sipr_range,confs.Q0Sipr_range)
        ...
        def get_pair(l_i_frame: list):
            ...
            MAX_PAIRS = confs.MAX_PAIRS  
            np.random.seed(confs.SEED)
            ...
            while (len(ret) < MAX_PAIRS and i_pair < len(pairs)):
                pair = pairs[i_pair]
                pair = tuple(pair)
                if (confs.Q0INDEX is not None):
                    Q0INDEX = confs.Q0INDEX
        ...
        if (evalResult.exist_pairs(sequence_name, pairs) and confs.SKIP_EVAL_SEQ_IF_EVAL_RESULT_EXIST):
        ...
        for i_pair, (i, j) in enumerate(pairs):
            Global.curPairStr=f"{category}-{sequence_name}-{l_i_frame[i]}-{l_i_frame[j]}"
            Global.curPairOnlyQ0Str=f"{category}-{sequence_name}-{l_i_frame[i]}"
            def eval_pair():
                nonlocal model_estimator_instance
                print(f"\n--------evaluating---{i_pair}/{len(pairs)}---(i, j)=({i},{j})---seq={sequence_name}---")
                if (evalResult.exist_pair(sequence_name, i, j) and confs.SKIP_EVAL_SEQ_IF_EVAL_RESULT_EXIST):
                    print(f"eval result exists, skip eval this pair. {i_pair=}")
                    # continue
                    return
                key_frames = [l_i_frame[i], l_i_frame[j]]
                Global.pairs_result.new_k(  (category,sequence_name,l_i_frame[i], l_i_frame[j],)  )
                if model_estimator_instance is None:
                    model_estimator_instance = Estimator4Co3dEval(
                        refIdWhenNormal=confs.RefIdWhenNormal.get_id(category, sequence_name,
                                                                            confs.refIdSuffix)
                        # output_dir=f'./co3d_SparseViewPoseEstimator_output/{id_}',
                    )
                Global.intermediate["E2VG"]["inter_img"].l = []
                if (confs.ONLY_GEN_DO_NOT_MEASURE and i_pair > 0):  
                    # continue
                    return
                print(f"evaluating {sequence_name} {i_pair}/{len(pairs)}")
                if confs.Q1Sipr:
                    q1ipr=map_string_to_int(category+sequence_name+str(key_frames[1]),
                                            -confs.Q1Sipr_range,confs.Q1Sipr_range)
                    print(f"add  inplane rotation  {q1ipr} to query image")
                else:
                    q1ipr=0
                if (model_name == "relpose++"):
                    batch = dataset.get_data_4relpose(sequence_name=sequence_name, index0=key_frames[0], index1=key_frames[1],q0ipr=q0ipr,q1ipr=q1ipr,)
                    crop_params = batch["crop_params"].to(device).unsqueeze(0)
                elif (model_name == "E2VG"):
                    batch = dataset.get_data_(sequence_name=sequence_name, ids=key_frames,q0ipr=q0ipr,q1ipr=q1ipr,)
                    Ks = batch["K"]
                ...
                R_gt_rel_opencv=pose44_gt_rel_opencv[:3,:3]
                ...
                def get_eleRadian(R, t):
                    ...
                    print("ele radian,degree:", eleRadian, eleRadian * 180 / math.pi)
                    return eleRadian
                ...
                if 1:
                    ...
                    refId1 = confs.RefIdWhenNormal.get_id(
                        category, sequence_name,
                        confs.refIdSuffix,
                        j
                    )
                    model_estimator_instance1 = Estimator4Co3dEval(
                        refIdWhenNormal=refId1,
                    )
                    if 1: # Gen2SM
                        R_pred_rel, T31_pred_rel, relative_pose, inter = infer_pair_latent.infer_in_latent_space(
                            estimator0=model_estimator_instance,
                            estimator1=model_estimator_instance1,
                            id0=model_estimator_instance.id_,
                            id1=refId1,
                            K0=K,
                            K1=K,
                            ...
                            bbox0=batch["bbox"][0],
                            bbox1=batch["bbox"][1],
                            R_GT_opencv=R_gt_rel_opencv,
                            elev_deg0=batch['elev_deg'][0],
                            elev_deg1=batch['elev_deg'][1],
                        )
                ...
                Global.pairs_result.set_cur_dic(  'R_error',R_error  )
                ...
                def _vis():
                    def vis_pose():
                        ...
                        l_color.append("grey")
                        title += f".elev={tmp_eleRadian * 180 / math.pi:.2f}°{q0ipr=}{q1ipr=}\nzero123 base img={Path(zero123_input_img_path).relative_to(confs.dataPath_rfdb)}"  # azim=-60, elev=30 by default
                        ...
                        # pose_save_path_format=osp.join(confs.evalResultPath_co3d, f"[{model_name}-{confs.idSuffix}]{category}-{sequence_name}-{key_frames[0]},{key_frames[1]} {{}}.npy")
                    ...
                    tmp_save_dir = osp.join(confs.evalVisPath,
                                            f"[{model_name}-{confs.idSuffix}]{category}-{sequence_name}")
                    os.makedirs(tmp_save_dir, exist_ok=True)
                    tmp_save_path = osp.join(tmp_save_dir,
                                             f"[{model_name}-{confs.idSuffix}]{category}-{sequence_name}-{key_frames[0]},{key_frames[1]}.jpg")
