...
class EvalResult():  
    ...
    class AllAcc:
        ...
        def dump_average_acc(SUFFIX="",only_log_these_cates__and__only_print_do_not_dump=None):
            ...
            def report(fullPaths,):
                ...
                OUT_NAME=f"[EvalResult.AllAcc{SUFFIX}]refIdSuffix={confs.refIdSuffix}.idSuffix={confs.idSuffix}.MAX_PAIRS={confs.MAX_PAIRS}.{f'{your_datetime():%m%d-%H-%M-%S}'}"
                while( os.path.exists(  os.path.join(confs.evalResultPath_co3d,f'{OUT_NAME}.xlsx')  )):
                    OUT_NAME+="_"
                df.to_excel(os.path.join(confs.evalResultPath_co3d,f'{OUT_NAME}.xlsx'))
                print("save to:\n",f'{OUT_NAME}.xlsx')
                if 0:
                    path=os.path.join(confs.path_4val,f'dic_avg{SUFFIX}.json')
    ...
    def __init__(self, model_name, idSuffix, category) -> None:
        ...
        #
        self.seqAndPair2err_json = osp.join(confs.evalResultPath_co3d,
        ...
        if (os.path.exists(self.seqAndPair2err_json)):
            ...
            self.seqAndPair2err = {}
        #
        self.category_acc_json = osp.join(confs.evalResultPath_co3d,
